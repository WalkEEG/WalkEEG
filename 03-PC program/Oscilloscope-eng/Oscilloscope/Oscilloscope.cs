...
namespace Oscilloscope
{
    public partial class Oscilloscope : Form
    {
            ...
            try
            {
                if (serialPort1.IsOpen)
                {
                   ...
                   while (handerListening) Application.DoEvents();//关闭串口前处理完消息队列中的所有事件
                    ...
                    if (!serialPort1.IsOpen)
                    {
                        buttonSwitch.Text = "Open the serial port";
                ...
                else
                {
                    ...
                    if (serialPort1.IsOpen)
                    {
                        buttonSwitch.Text = "Close the serial port";
        ...
        void paint_ax(double x,double y,double z)
        {
            ...
            try
            {
                ...
                if (checkBox1.Checked == true)
                {
                    ...
                    chart1.Series["Channel 1"].Points.AddXY(pointIndex, x);
                }
                if (checkBox2.Checked == true)
                {
                    chart1.Series["Channel 2"].Points.AddXY(pointIndex, y);
                }
                if (checkBox3.Checked == true)
                {
                    chart1.Series["Channel 3"].Points.AddXY(pointIndex, z);
                }
                pointIndex++;
        ...
        {
            chart1.Series["Channel 1"].Points.Clear();
            chart1.Series["Channel 2"].Points.Clear();
            chart1.Series["Channel 3"].Points.Clear();
            pointIndex = 0;
            axisChange = false;
        }
        #endregion
